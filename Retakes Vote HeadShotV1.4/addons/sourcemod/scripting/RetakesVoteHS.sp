...
#pragma tabsize 0
#define DEBUG
#define PREFIX " \x04[VoteHeadShot]\x01"
...
{
	name = "Retakes Only HeadShot Vote",
	author = "SheriF",
	description = "Simple Only HeadShot Vote for Retakes servers",
	version = "1.4",
	url = ""
};
...
public void OnPluginStart()
{
    ...
    RegConsoleCmd("sm_vh", VoteHs, "Enable/Disable your decision for the Vote HeadShot.");
    RegConsoleCmd("sm_voteheadshot", VoteHs, "Enable/Disable your decision for the Vote HeadShot.");
    RegConsoleCmd("sm_votehs", VoteHs, "Enable/Disable your decision for the Vote HeadShot.");
    RegConsoleCmd("sm_cvh", CVoteHs, "Showing the currrent status of the Vote HeadShot.");
    RegAdminCmd("sm_fvh", FVoteHs,ADMFLAG_GENERIC, "Force Enable/Disable Only HeadShot.");
...
public Action OnRoundStart(Event event, const char[] name, bool dontBroadcast)
{
	if(IsActive&&g_cvAutoDisable.IntValue==-1)
	{
		...
		CPrintToChatAll("%s Only HeadShot is \x04Activated!\x01",PREFIX);
		PrintCenterTextAll("Only HeadShot is Activated!");
	}
	else if(!IsActive)
	...
	if(IsActive)
	{
		...
		{
			SetConVarInt(mp_damage_headshot_only, 0);
			CPrintToChatAll("%s Only HeadShot is \x07Disabled!\x01",PREFIX);
			PrintCenterTextAll("Only HeadShot is Disabled!");
			IsActive = false;
			RoundsCounter = 0;
		}
		else if(g_cvAutoDisable.IntValue!=-1)
		{
			RoundsCounter++;
			SetConVarInt(mp_damage_headshot_only, 1);
			CPrintToChatAll("%s Only HeadShot will be \x04Activated\x01 for \x07%i\x01 more rounds.",PREFIX,1+g_cvAutoDisable.IntValue-RoundsCounter);
			PrintCenterTextAll("Only HeadShot will be Activated for %i more rounds.",1+g_cvAutoDisable.IntValue-RoundsCounter);
		}		
	}
}
